...
export async function verifyPassword(password: string, hashedPassword: string): Promise<boolean> {
  ...
  // Legacy plaintext passwords are no longer supported — force re-hash via admin setup
...
async function getAdminCredentials(): Promise<{ username: string; password: string } | null> {
  try {
    ...
    logError('Error fetching admin credentials:', error)
  }
...
export async function authenticateAdmin(
  ...
  if (!creds) {
    logError('❌ Admin credentials not configured in database!')
    await auditLog({
      action: 'login_failed',
      category: 'auth',
      ipAddress: ip,
  ...
  // SECURITY: Auto-migration of plaintext passwords removed.
  // If password is not bcrypt-hashed, login will fail — admin must re-setup credentials.
