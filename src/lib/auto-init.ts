...
export async function initializeDatabase(): Promise<{ success: boolean; message: string }> {
  ...
  try {
    log('[AUTO-INIT] Starting database initialization...')
    
    // Create all tables
    for (let i = 0; i < ALL_TABLES.length; i++) {
      try {
        await sqlClient.unsafe(ALL_TABLES[i])
        log(`[AUTO-INIT] Created table ${i + 1}/${ALL_TABLES.length}`)
      } catch (error: any) {
        // Ignore "already exists" errors
        if (!error.message?.includes('already exists')) {
          logError(`[AUTO-INIT] Error creating table ${i + 1}:`, error.message)
        }
      }
    }
    ...
    for (const indexSql of INDEXES) {
      try {
        ...
        // Ignore index errors
        if (!error.message?.includes('already exists')) {
          logError('[AUTO-INIT] Index error:', error.message)
        }
      }
    }
    ...
    try {
      ...
      if (settingsCheck.length === 0) {
        await sqlClient`
          ...
          VALUES (1, 'My Shop', 'Fresh from farm to your table', '60', '120', '500')
          ON CONFLICT (id) DO NOTHING
        `
        log('[AUTO-INIT] Created default settings row')
      }
    } catch (error: any) {
      logError('[AUTO-INIT] Error creating default settings:', error.message)
    }
    
    globalThis.__dbInitialized = true
    globalThis.__dbInitializing = false
    
    log('[AUTO-INIT] ✅ Database initialization complete!')
    
    return { success: true, message: 'Database initialized successfully' }
    
  } catch (error: any) {
    globalThis.__dbInitializing = false
    logError('[AUTO-INIT] ❌ Database initialization failed:', error.message)
