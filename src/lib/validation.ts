...
// Rate limiting - hybrid: in-memory fast layer + DB-backed persistent layer
// In-memory for speed, DB survives restarts
const rateLimitStore = new Map<string, { count: number; resetAt: number }>()

// In-memory rate limiter (fast, resets on restart — use for non-critical routes)
...
// DB-backed rate limiter (persistent, survives restarts — use for critical routes)
import { db } from '@/db'
...
export async function checkRateLimitDB(
  ...
  try {
    ...
    await db.execute(sql`
      DELETE FROM api_rate_limits WHERE reset_at < ${now}
    `)

    // Get existing record
    const existing = await db.execute(sql`
      SELECT count, reset_at FROM api_rate_limits WHERE key = ${key}
    `) as any[]

    if (existing.length === 0) {
      // New entry
      await db.execute(sql`
        INSERT INTO api_rate_limits (key, count, reset_at) VALUES (${key}, 1, ${resetAt})
      `)
      return { allowed: true, remaining: maxRequests - 1, resetAt }
    }
    ...
    if (currentCount >= maxRequests) {
      return { allowed: false, remaining: 0, resetAt: Number(record.reset_at) }
    }
    ...
    return { allowed: true, remaining: maxRequests - currentCount - 1, resetAt: Number(record.reset_at) }
  } catch (error) {
    // Fallback to in-memory if DB fails
    console.error('[RateLimit] DB error, falling back to in-memory:', error)
