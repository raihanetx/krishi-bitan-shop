 ...
 * Never stored in database or exposed via API.
 */
...
class SteadfastService {
  ...
  private async getCredentials(): Promise<CourierCredentials | null> {
    ...
    // Read from environment variables — never from database
    const apiKey = process.env.STEADFAST_API_KEY
    const secretKey = process.env.STEADFAST_SECRET_KEY
    const webhookUrl = process.env.STEADFAST_WEBHOOK_SECRET || ''

    if (apiKey && secretKey) {
      const creds: CourierCredentials = { apiKey, secretKey, webhookUrl }
  ...
  async createBulkOrders(orders: BulkOrderItem[]): Promise<BulkOrderResponse[]> {
    ...
    try {
      ...
      logError('[Steadfast] Bulk order error:', error)
      return []
    }
  }
  ...
  async getStatusByConsignmentId(consignmentId: number): Promise<any> {
    ...
    try {
      ...
      logError('[Steadfast] Get status error:', error)
      return null
    }
  }
  ...
  async getStatusByInvoice(invoice: string): Promise<any> {
    ...
    try {
      ...
      logError('[Steadfast] Get status by invoice error:', error)
      return null
    }
  }
  ...
  async getStatusByTrackingCode(trackingCode: string): Promise<any> {
    ...
    try {
      ...
      logError('[Steadfast] Get status by tracking code error:', error)
      return null
    }
  }
  ...
  async getBalance(): Promise<{ status: number; current_balance: number } | null> {
    ...
    try {
      ...
      logError('[Steadfast] Get balance error:', error)
      return null
    }
  }
}
