...
function getEncryptionKey(): Buffer {
  ...
  if (directKey && directKey.length === 64) {
    try {
      ...
      logError('Invalid ENCRYPTION_KEY format - must be 64 hex characters')
    }
  }
...
export function decrypt(encryptedText: string): string {
  ...
  if (parts.length !== 4) {
    logError('Decryption failed: Invalid format')
    return ''
  }
  ...
  try {
    ...
    logError('Decryption failed - encryption key may have changed:', error)
    return ''
  }
}
...
export function safeDecrypt(encryptedText: string): string | null {
  ...
  if (parts.length !== 4) {
    logError('Safe decrypt failed: Invalid format')
    return null
  }
  ...
  try {
    ...
    logError('Safe decrypt failed - encryption key may have changed:', error)
    return null
  }
}
...
export async function recordFailedAttemptDB(ip: string): Promise<{ count: number; lockedUntil: Date | null }> {
  try {
    ...
    logError('Rate limit DB error:', error)
...
export async function clearFailedAttemptsDB(ip: string): Promise<void> {
  try {
    ...
    logError('Failed to clear attempts:', error)
  }
}
...
export async function clearAllRateLimits(): Promise<void> {
  try {
    ...
    log('✅ All rate limits cleared')
  } catch (error) {
    logError('Failed to clear all rate limits:', error)
  }
}
