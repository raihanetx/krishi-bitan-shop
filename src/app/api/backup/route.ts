...
const BACKUP_TABLES = {
  ...
  settings: { table: settings, excludeFields: ['adminPassword', 'steadfastApiKey', 'steadfastSecretKey', 'steadfastWebhookUrl', 'cloudinaryCloudName', 'cloudinaryApiKey', 'cloudinaryApiSecret'] },
...
export async function GET(request: NextRequest) {
  try {
    ...
    log('[BACKUP] Starting backup process...')
    
    const backupData: Record<string, any> = {
      _meta: {
        version: '1.0.0',
        exportedAt: new Date().toISOString(),
        exportedBy: 'Shop Backup System',
    ...
    for (const [tableName, config] of Object.entries(BACKUP_TABLES)) {
      try {
        ...
        log(`[BACKUP] ${tableName}: ${data.length} records`)
      } catch (tableError) {
        logError(`[BACKUP] Error fetching ${tableName}:`, tableError)
    ...
    log(`[BACKUP] Backup complete. Total size: ${jsonString.length} bytes`)
    ...
    logError('[BACKUP] Backup failed:', error)
