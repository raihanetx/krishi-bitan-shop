...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [allOrders, allProductViews, allOrderItems, allProducts, cartAdditions, allVisitorSessions, allAbandonedCheckouts] = await Promise.all([
      ...
      db.select().from(visitorSessions).where(gte(visitorSessions.date, startDateStr)),
      db.select().from(abandonedCheckouts).where(gte(abandonedCheckouts.visitDate, startDateStr)),
    ])
