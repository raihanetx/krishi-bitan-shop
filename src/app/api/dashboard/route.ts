...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      log('[DASHBOARD] Some tables may not exist yet, returning empty data')
    }
    ...
    if (DATABASE_URL) {
      try {
        ...
        logError('Session analytics query error:', sessionError)
    ...
    if (DATABASE_URL) {
      try {
        ...
        logError('Cart actions per product query error:', cartError)
      }
    }
    ...
    if (periodVisitorIds.length > 0 && DATABASE_URL) {
      try {
        ...
        log('📊 [DASHBOARD] Visitor counts (corrected):', { 
          uniqueVisitors, 
          newVisitorsCount, 
          returningVisitorsCount,
          totalVisits,
        ...
        logError('Visitor counting error:', visitorError)
