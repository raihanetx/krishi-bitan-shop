...
import { checkRateLimitDB, rateLimitErrorResponse } from '@/lib/validation'
...
async function sendToSteadfastCourier(order: any): Promise<{ success: boolean; consignmentId?: number; trackingCode?: string; error?: string }> {
  try {
    ...
    logError('Steadfast API error:', result)
    return { success: false, error: result.message || 'Failed to create consignment' }
  } catch (error) {
    logError('Error sending to Steadfast:', error)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (phone) {
      ...
      const rateLimit = await checkRateLimitDB(`orders-phone:${ip}`, 30, 60000) // 30 requests per minute
      ...
      // Log for debugging
      log('[ORDERS API] Public fetch for phone:', cleanPhone)
      ...
      if (customerOrders.length === 0 && cleanPhone.startsWith('0')) {
        ...
        log('[ORDERS API] Trying without leading 0:', phoneWithoutLeadingZero)
      ...
      if (customerOrders.length === 0 && !cleanPhone.startsWith('0')) {
        ...
        log('[ORDERS API] Trying with leading 0:', phoneWithLeadingZero)
        customerOrders = await db.select().from(orders).where(eq(orders.phone, phoneWithLeadingZero))
      }
      
      log('[ORDERS API] Found', customerOrders.length, 'orders for phone:', cleanPhone)
      ...
      log('[ORDERS API] Returning orders with statuses:', ordersWithItems.map(o => ({ id: o.id, status: o.status, courierStatus: o.courierStatus })))
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!honeypotCheck.valid) {
      ...
      log('[SECURITY] Bot detected via honeypot')
      return NextResponse.json(
    ...
    if (botCheck.shouldBlock) {
      log('[SECURITY] Bot blocked:', botCheck.reasons)
      return NextResponse.json(
    ...
    const rateLimit = await checkRateLimitDB(rateLimitKey, 10, 60000) // 10 orders per minute per IP
...
export async function PUT(request: NextRequest) {
  try {
    ...
    logError('Error updating order items:', error)
    return NextResponse.json(
      ...
      if (orderResult.length > 0) {
        ...
        if (!order.consignmentId) {
          ...
          if (courierResult.success) {
            ...
            log(`Order ${id} sent to Steadfast Courier: Consignment ${courierResult.consignmentId}`)
          } else {
            log(`Failed to send order ${id} to courier: ${courierResult.error}`)
          }
        }
      }
    }
