 ...
 * 
 * SECURITY: Requires STEADFAST_API_KEY environment variable
 */
...
/**
 * Get courier API key from environment variable only
 */
function getCourierApiKey(): string | null {
  return process.env.STEADFAST_API_KEY || null
}
...
export async function POST(request: NextRequest) {
  try {
    // RATE LIMITING: Prevent brute-force attacks on webhook token
    const ip = request.headers.get('x-forwarded-for')?.split(',')[0]?.trim()
      || request.headers.get('x-real-ip')
      || 'unknown'
    const { checkRateLimit } = await import('@/lib/validation')
    const rateLimit = checkRateLimit(`webhook:${ip}`, 60, 60000) // 60 req/min
    ...
    if (!verification.valid) {
      logError('[Webhook] Rejected:', verification.reason)
      return NextResponse.json(
    ...
    log('[Webhook] Received from Steadfast:', JSON.stringify(body, null, 2))
    ...
    if (!order) {
      log('[Webhook] Order not found for:', body.consignment_id || body.invoice)
      return NextResponse.json(
    ...
    if (body.notification_type === 'delivery_status') {
      ...
      log(`[Webhook] Updated order ${order.id} status to ${courierStatus}`)
    ...
    if (body.notification_type === 'tracking_update') {
      log(`[Webhook] Tracking update for order ${order.id}: ${body.tracking_message}`)
    ...
    logError('[Webhook] Error processing webhook:', error)
    return NextResponse.json(
