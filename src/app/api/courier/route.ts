...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logError('Courier API error:', error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    log('[Courier] Original phone:', order.phone)
    log('[Courier] Cleaned phone:', phone)
    if (phone.length > 11) {
      phone = phone.slice(-11) // Take last 11 digits
    } else if (phone.length < 11) {
      phone = phone.padStart(11, '0')
    }
    log('[Courier] Final phone:', phone)
    ...
    log('[Courier] Sending to Steadfast:', JSON.stringify(orderData, null, 2))
    ...
    logError('Create courier order error:', error)
    return NextResponse.json(
      { success: false, error: 'Failed to process request' },
      { status: 500 }
    )
