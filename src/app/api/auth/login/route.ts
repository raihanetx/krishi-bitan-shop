...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!honeypotCheck.valid) {
      ...
      log('[SECURITY] Bot detected on login via honeypot')
    ...
    if (botCheck.shouldBlock) {
      log('[SECURITY] Bot blocked on login:', botCheck.reasons)
      return NextResponse.json({ 
        success: false, 
        error: 'Invalid credentials'
      }, { status: 401 })
    ...
    if (result.success && result.token) {
      ...
      log('[LOGIN] Session cookie set, duration:', SESSION_DURATION_SECONDS, 'seconds (7 days), secure:', isProduction)
      return response
    }
