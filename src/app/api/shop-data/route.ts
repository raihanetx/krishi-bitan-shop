...
export async function GET() {
  ...
  try {
    ...
    if (settingsData[0]) {
      ...
      settingsObj = {
        websiteName: s.websiteName || 'My Shop',
        slogan: s.slogan ?? '',
    ...
    logError('Shop data fetch error:', error)
...
export async function POST() {
  try {
    ...
    log('[SHOP-DATA] Cache cleared by admin')
    ...
    logError('[SHOP-DATA] Error clearing cache:', error)
    return NextResponse.json({
      success: false,
