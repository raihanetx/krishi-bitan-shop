...
export async function POST(request: NextRequest) {
  try {
    ...
    log('[RESTORE] Starting restore process...')
    ...
    log(`[RESTORE] Backup version: ${backupData._meta.version}`)
    log(`[RESTORE] Exported at: ${backupData._meta.exportedAt}`)

    // Start transaction-like operation
    const results: Record<string, { deleted: number; inserted: number }> = {}

    // Step 1: Delete existing data in reverse dependency order
    log('[RESTORE] Step 1: Clearing existing data...')
    ...
    try {
      ...
      if (backupData.orderItems && backupData.orderItems.length > 0) {
        ...
        log(`[RESTORE] Deleted order items`)
      }
      ...
      if (backupData.relatedProducts && backupData.relatedProducts.length > 0) {
        ...
        log(`[RESTORE] Deleted related products`)
      }
      ...
      if (backupData.productFaqs && backupData.productFaqs.length > 0) {
        ...
        log(`[RESTORE] Deleted product FAQs`)
      }
      ...
      if (backupData.productImages && backupData.productImages.length > 0) {
        ...
        log(`[RESTORE] Deleted product images`)
      }
      ...
      if (backupData.categories && backupData.categories.length > 0) {
        ...
        log(`[RESTORE] Deleted categories`)
      }
      ...
      if (backupData.abandonedCheckouts && backupData.abandonedCheckouts.length > 0) {
        ...
        log(`[RESTORE] Deleted abandoned checkouts`)
      }

    } catch (deleteError) {
      logError('[RESTORE] Error during deletion:', deleteError)
      return NextResponse.json({ 
        success: false, 
    ...
    log('[RESTORE] Step 2: Inserting new data...')
    ...
    try {
      ...
      if (backupData.categories && backupData.categories.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanCategories.length} categories`)
      }
      ...
      if (backupData.products && backupData.products.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanProducts.length} products`)
      }
      ...
      if (backupData.variants && backupData.variants.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanVariants.length} variants`)
      }
      ...
      if (backupData.productImages && backupData.productImages.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanImages.length} product images`)
      }
      ...
      if (backupData.productFaqs && backupData.productFaqs.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanFaqs.length} product FAQs`)
      }
      ...
      if (backupData.relatedProducts && backupData.relatedProducts.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanRelated.length} related products`)
      }
      ...
      if (backupData.customers && backupData.customers.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanCustomers.length} customers`)
      }
      ...
      if (backupData.reviews && backupData.reviews.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanReviews.length} reviews`)
      }
      ...
      if (backupData.orders && backupData.orders.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanOrders.length} orders`)
      }
      ...
      if (backupData.orderItems && backupData.orderItems.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanOrderItems.length} order items`)
      }
      ...
      if (backupData.coupons && backupData.coupons.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanCoupons.length} coupons`)
      }
      ...
      if (backupData.abandonedCheckouts && backupData.abandonedCheckouts.length > 0) {
        ...
        log(`[RESTORE] Inserted ${cleanAbandoned.length} abandoned checkouts`)
      }
      ...
      if (backupData.settings && backupData.settings.length > 0) {
        ...
        log(`[RESTORE] Updated settings`)
      }

    } catch (insertError) {
      logError('[RESTORE] Error during insertion:', insertError)
      return NextResponse.json({ 
        success: false, 
    ...
    logError('[RESTORE] Restore failed:', error)
    return NextResponse.json({ 
      success: false, 
