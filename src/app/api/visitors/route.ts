...
export async function POST(request: NextRequest) {
  try {
    ...
    log('📥 [VISITORS API] Received:', { visitorId, sessionId, deviceType, browser, os, date })
    ...
    if (existingSession.length > 0) {
      log('✅ [VISITORS API] Session already tracked today')
      return NextResponse.json({ 
        success: true, 
    ...
    if (visitorId) {
      ...
      if (allPreviousVisits.length > 0) {
        ...
        isActuallyNew = false
        log('🔄 [VISITORS API] Returning visitor:', visitorId, '| First visit:', firstVisitDate)
      } else {
        // Never seen before - this is a NEW visitor
        isActuallyNew = true
        log('🆕 [VISITORS API] New visitor:', visitorId)
      }
    } else {
      // No visitorId provided - treat as new session without persistent tracking
      log('⚠️ [VISITORS API] No visitorId provided, treating as new')
    }
    ...
    logError('[VISITORS API] Error:', error)
    return NextResponse.json({
      success: false,
...
export async function GET(request: NextRequest) {
  try {
    ...
    logError('Visitor stats error:', error)
    return NextResponse.json({
      success: false,
